...
% 输入层
\node[box, fill=green!20] (input1) at (0,3) {$x_i$};
\node[box, fill=red!20] (input2) at (0,-3) {$x_j$};

% 共享网络（上部）
\node[bigbox] (net1) at (3.5,3) {};
\node[align=center] at (3.5,3) {\textbf{共享网络}\\$f_\theta$\\\\Conv Layers\\FC Layers};

% 共享网络（下部）
\node[bigbox] (net2) at (3.5,-3) {};
\node[align=center] at (3.5,-3) {\textbf{共享网络}\\$f_\theta$\\\\Conv Layers\\FC Layers};
...
% 特征输出
\node[box, fill=yellow!30] (feat1) at (7,3) {$z_i$};
\node[box, fill=yellow!30] (feat2) at (7,-3) {$z_j$};

% 距离层
\node[draw, thick, rounded corners, fill=orange!20, minimum width=2cm, minimum height=1.2cm] (dist) at (10,0) {\textbf{距离层}\\$d = \|z_i - z_j\|$};

% 输出
\node[box, fill=purple!20] (output) at (14,0) {相似度\\分数};
...
% ===== 左侧：网络架构 =====
\node[font=\bfseries] at (3,6) {Triplet网络架构};

% 输入
\node[box, fill=gray!25] (anchor) at (0,3) {锚点\\$x_a$};
\node[box, fill=green!25] (pos) at (0,-1) {正样本\\$x_p$};
\node[box, fill=red!25] (neg) at (0,-5) {负样本\\$x_n$};

% 共享网络
\node[bigbox] (net1) at (3.5,3) {};
\node[align=center] at (3.5,3) {\textbf{共享网络}\\$f_\theta$};

\node[bigbox] (net2) at (3.5,-1) {};
\node[align=center] at (3.5,-1) {\textbf{共享网络}\\$f_\theta$};

\node[bigbox] (net3) at (3.5,-5) {};
\node[align=center] at (3.5,-5) {\textbf{共享网络}\\$f_\theta$};

% 特征
\node[box, fill=yellow!30] (za) at (6.5,3) {$z_a$};
\node[box, fill=yellow!30] (zp) at (6.5,-1) {$z_p$};
\node[box, fill=yellow!30] (zn) at (6.5,-5) {$z_n$};

% 损失
\node[draw, thick, rounded corners, fill=orange!20, minimum width=3cm, minimum height=1.5cm, align=center] (loss) at (12,-1) {\textbf{Triplet Loss}\\$\mathcal{L} = \max(0, d(z_a,z_p) - d(z_a,z_n) + m)$};
...
% ===== 右侧：嵌入空间示意 =====
\node[font=\bfseries] at (20,6) {嵌入空间示意};

% 坐标轴
\draw[->] (17, -3) -- (17, 4) node[above] {$z_2$};
\draw[->] (17, -3) -- (24, -3) node[right] {$z_1$};

% 锚点
\node[dot, fill=black] (a) at (22,1) {};
\node[above] at (a) {$z_a$ (锚点)};

% 正样本（靠近锚点）
\node[dot, fill=green!60] (p) at (19,0.5) {};
\node[below left] at (p) {$z_p$ (正样本)};
\draw[->, thick, green!60!black] (a) -- (p);
\node[below, sloped, font=\small] at ($(a)!0.5!(p)$) {$d(z_a,z_p)$小};

% 负样本（远离锚点）
\node[dot, fill=red!60] (n) at (22.5,-2) {};
...
\node[blue, font=\small] at (21.2, 2.2) {margin $m$};

% 约束说明
\node[align=left, font=\small] at (20.5, -4) 
