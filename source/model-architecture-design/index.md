...
本章聚焦于 **CNN（卷积神经网络）** 的架构设计。虽然部分原则（如跳跃连接、注意力）具有跨架构的通用性，但感受野、空洞卷积、DW 卷积等策略是 CNN 特有的。序列处理（RNN、Transformer）和生成模型（扩散模型、GAN）的架构设计不在本章讨论范围内——详见 {ref}`sequence-modeling`。
```
