...
{doc}`computational-graph`中介绍的计算图描述了数据如何在模型中流动。但数据流向什么样的目标？不同类型的"规律"需要不同的数学建模方式。本节从"找规律"的视角，理解深度学习中的三类核心任务。

## 引言：机器学习在找什么规律？

机器学习的本质是从数据中**发现规律**，然后用这些规律做预测。但"规律"有不同形式：
...
\begin{tikzpicture}[scale=0.9]
    ...
    % 数据点
    \foreach \x/\y in {0.5/0.8, 1/1.5, 1.5/1.3, 2/2.5, 2.5/2.2, 3/3.5, 3.5/3.0, 4/4.2, 4.5/3.8, 5/4.8}
        \fill[blue!60] (\x, \y) circle (2.5pt);
    
    % 拟合直线 y = 0.9x + 0.5
    \draw[thick, red] (0, 0.5) -- (5.5, 5.45);
    \node[red] at (5.5, 4.5) {$\hat{y} = wx + b$};
...
\begin{tikzpicture}[scale=0.7]
    ...
    \begin{scope}[shift={(5,0)}]
        \draw[->] (-0.3,0) -- (3,0) node[right] {$x$};
        \draw[->] (0,-0.3) -- (0,3) node[above] {$y$};
        \foreach \x/\y in {0.2/0.3, 0.6/1.5, 1.0/2.5, 1.4/2.8, 1.8/2.2, 2.2/1.0, 2.6/0.5}
            \fill[blue!60] (\x, \y) circle (2pt);
        \draw[thick, red, domain=0:2.8, smooth] plot (\x, {0.5 + 2.5*sin(\x r)});
