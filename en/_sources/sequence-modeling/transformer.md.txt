...
如果你已经学过 {doc}`../cnn-expedition/practice-peak/cnn-basics`，这里有一个重要的联系：**自注意力在表达能力上是卷积的严格超集**。
...
~~~{note}
**ViT 为什么能成功？** {doc}`../cnn-expedition/practice-peak/cnn-basics` 告诉我们，CNN 成功的核心是 {ref}`inductive-bias`——局部感受野和权值共享。自注意力放弃了这些硬编码的先验，换来了更大的灵活性。在数据量充足时，灵活性胜出（ViT 超越 CNN）；在数据量不足时，CNN 的先验仍然宝贵（CNN 在小数据集上更好）。
...
```{note}
**Transformer 和 {doc}`../cnn-expedition/image-net-era/index` 中的注意力是同一种"注意力"吗？**
...
- 注意力和 {doc}`../cnn-expedition/image-net-era/index` 中的 CNN 注意力是不同的概念——一个在序列维度上交互，一个在通道/空间维度上重加权
