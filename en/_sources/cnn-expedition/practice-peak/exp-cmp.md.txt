...
{doc}`introduction` 中我们讨论了 **归纳偏置** {cite}`mitchell1980need`——CNN 通过局部感受野和权值共享引入"空间结构"的先验，全连接网络没有。{doc}`fc-layer-basics` 和 {doc}`cnn-basics` 展示了两种架构的实现，{doc}`le-net` 完成了练习峰首登。
