...
{doc}`../image-net-era/index` 详细介绍了 SE-Net 和 CBAM 的具体实现。现在我们从**设计心法**的角度来理解——注意力不是一种"技巧"，而是神经网络中**信息路由**的通用机制。

## 重新理解注意力：从"模块"到"路由"

{doc}`../practice-peak/cnn-basics` 告诉我们卷积有 {ref}`inductive-bias`——平移等变性、局部感受野。这些归纳偏置让 CNN 很高效，但也带来了一个代价：**所有输入位置被同等对待**。
...
{doc}`../image-net-era/se-net` 详细介绍了 SE-Net 的三步机制（压缩→激励→缩放）。从架构设计角度，通道注意力的核心价值在于：**用极小的额外代价（<3% 参数），实现全局上下文对局部特征的动态调控**。
...
{doc}`../image-net-era/cbam` 展示了如何用通道压缩和 7×7 卷积生成空间注意力图。
...
知道看哪了。但背包太重，爬不动。第四件装备——{doc}`part5-efficiency`。
