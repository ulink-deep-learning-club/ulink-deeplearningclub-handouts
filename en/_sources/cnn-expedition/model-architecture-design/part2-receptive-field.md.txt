...
{ref}`receptive-field` 告诉我们：卷积层越深，感受野越大。但 {doc}`../image-net-era/inception` 揭示了一个根本问题——**每一层只有一个固定大小的感受野**。
...
这是 {doc}`../image-net-era/inception` 的核心思想，但我们从**设计心法**而非架构本身来理解它。
...
**代价与缓解**：参数和计算量增加（多个分支同时跑）。{doc}`../image-net-era/inception` 中的关键创新是用 **1×1 卷积先降维**——在昂贵的 3×3 和 5×5 卷积之前，先用 1×1 把通道数降下来：
...
**本质**：用稀疏采样替代密集采样。3×3 卷积 + 空洞率 2 = 5×5 的感受野，但参数还是 9 个。{doc}`../image-net-era/inception` 中我们已用 TikZ 图展示了普通卷积与空洞卷积的直观对比，这里不再重复。
...
```{mermaid}
flowchart TD
    START[任务类型?] --> DET{检测/分割?}
    
    DET -->|是| SCALE{多尺度目标明显?}
    SCALE -->|是| FPN[FPN/PAN]
    SCALE -->|否| RESOURCE{计算资源紧张?}
    RESOURCE -->|是| DILATED[空洞卷积]
    RESOURCE -->|否| FIXED{固定尺度目标?}
    FIXED -->|是| NORMAL[普通卷积]
    FIXED -->|否| FPN
    
    DET -->|否| CLS_SCALE{尺度变化大?}
    CLS_SCALE -->|是| INCEPTION[Inception/多尺度池化]
    CLS_SCALE -->|否| STANDARD[标准卷积]
```
...
望远镜调好了。但看到的东西，能传回来吗？第二件装备——{doc}`part3-depth-connection`。
