...
{doc}`../cnn-expedition/practice-peak/cnn-basics` 中我们讨论了 {ref}`inductive-bias`：CNN 通过架构设计将"局部相关性"的先验知识内置到模型中。预训练模型则更进一步——它不仅包含架构设计，还包含从海量数据中学到的**权重参数**，这些参数编码了边缘、纹理、形状等通用视觉特征。
