...
即使有了跨平台格式，模型本身的体积和推理速度仍然是一个问题。一个典型的 ResNet-50 约 100MB，在手机上跑一次推理可能需要数百毫秒。{doc}`model-optimization`会介绍量化和剪枝两种核心技术——用 4× 的模型压缩和 2-4× 的推理加速，让同一模型适配从云端到嵌入式的各种场景。
...
graph LR
    ...
    B --> C[模型优化<br/>量化与剪枝]
    C --> D[服务架构<br/>同步 vs 异步]
    D --> E[Ferrinx实践<br/>部署与操作]
    E --> F[安全与运维<br/>鉴权限流]
```
...
有了 ONNX 模型后，{doc}`model-optimization`会教你用量化和剪枝把这个模型变得"又轻又快"——这是让模型真正适配生产环境的关键一步。
