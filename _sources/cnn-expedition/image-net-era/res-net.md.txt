...
{doc}`inception` 用多尺度工具赢得了 ImageNet 2014——22 层。很自然的想法：堆更深，比如 50 层、100 层，会不会更好？

{ref}`receptive-field` 中讨论过：层数越多，感受野越大，每个神经元能捕获的上下文越丰富。理论上，100 层应该比 22 层更强。
...
**右图（SE-ResNet 模块）**：在残差连接基础上增加了 Squeeze-and-Excitation 分支，用于学习通道注意力——自动判断"哪些特征通道更重要"。这是{doc}`se-net`的内容，展示了 ResNet 架构的扩展性。图片来自 SE-Net 论文 {cite}`hu2018squeeze`。
```
