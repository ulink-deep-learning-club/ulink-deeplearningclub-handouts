...
{doc}`alexnet` 用 11×11 的大卷积核首登了 ImageNet。一种冰镐打天下——太粗暴了。
...
用更专业的术语说，这就是**最大化输入与输出的互信息**——让网络尽可能多地保留原始图像的信息（这一概念将在 {doc}`../model-architecture-design/part1-intro` 中详细展开）。单分支会"丢失"某些尺度的信息，而多分支确保了**每个尺度都有专门的通道来处理**。
