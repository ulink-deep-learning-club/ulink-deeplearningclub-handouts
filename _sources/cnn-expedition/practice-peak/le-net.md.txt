...
{doc}`cnn-basics` 给了我们冰镐和绳索——局部感受野、权值共享、参数效率。但知道工具有什么用，和真正用它们爬上一座山，是两回事。本节分析 **LeNet-5**——**第一个成功的 CNN 架构**，看这些理论如何转化为工程实践。
...
登顶了。但登山者不会只靠感觉——{doc}`exp-cmp`：把赤手空拳和冰镐绳索放在一起，数据说话。
