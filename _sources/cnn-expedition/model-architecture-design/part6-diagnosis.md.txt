...
上述诊断流程给出了**假设**（"可能是容量不够"），但如何**验证**这个假设？这就是 {doc}`../ablation-study/index` 的价值所在。
...
**关键原则**（来自 {doc}`../ablation-study/experiment-design`）：
...
更多消融实验的方法论，参见 {doc}`../ablation-study/introduction` 和 {doc}`../ablation-study/experiment-design`。
...
{doc}`../image-net-era/res-net` 展示了退化问题：56 层网络比 20 层更差。
...
GoogLeNet 6.8M 参数，远少于 VGG 的 138M，但效果更好。{doc}`../image-net-era/inception` 已经展示了这点。
