...
{doc}`../image-net-era/res-net` 揭露了一个惊人的事实：堆更多的层不一定更好，反而可能更差。根源在于{ref}`gradient-vanishing`——梯度在反向传播中逐层衰减，浅层收不到训练信号。
...
ResNet 的核心原理和梯度推导已在 {doc}`../image-net-era/res-net` 中详细介绍。从架构设计角度，我们提炼为一条核心原则：
...
通路畅通了。但网络知道该看哪吗？第三件装备——{doc}`part4-attention`。
