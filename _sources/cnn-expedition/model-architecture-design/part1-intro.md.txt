...
{doc}`../image-net-era/index` 翻完了。AlexNet 的首登、Inception 的多尺度、ResNet 的补给站、SE-Net/CBAM 的聚焦、MobileNet/EfficientNet 的预算学——八件装备，八年。
...
| 零件 | 来源 | 解决什么问题 |
| ---------- | ---------- | ---------- |
| 卷积（不同大小） | {doc}`../practice-peak/cnn-basics` | 提取局部特征 |
| 池化 | {doc}`../practice-peak/le-net` | 降采样、增大感受野 |
| 跳跃连接 | {doc}`../image-net-era/res-net` | 让梯度能传回浅层 |
| 多分支并行 | {doc}`../image-net-era/inception` | 同时看多个尺度 |
| 通道注意力 | {doc}`../image-net-era/se-net` | 知道哪个特征重要 |
| 空间注意力 | {doc}`../image-net-era/cbam` | 知道哪里重要 |
...
第一件装备。望远镜——{doc}`part2-receptive-field`。
