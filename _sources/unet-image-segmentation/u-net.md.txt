...
编码器和 {doc}`../cnn-expedition/practice-peak/le-net` 前半部分几乎一样——重复的卷积块 + 最大池化。
...
这就是跳跃连接被称为"梯度高速公路"的原因——它为梯度提供了**绕过深度、直达浅层**的捷径。这个思路与 ResNet {cite}`he2016deep` 的残差连接一脉相承：都是通过短路连接为梯度创造高速公路。{doc}`../cnn-expedition/ablation-study/experiment-design` 中的消融实验也可以验证：去掉跳跃连接后，深层网络的浅层几乎学不到东西。
