...
{doc}`../cnn-expedition/practice-peak/cnn-basics` 中我们学的 CNN 擅长做**图像分类**：输入一张 28×28 的手写数字，输出"这是 3"。关键在于，分类只需要回答"整体是什么"——即使你只看到数字的一小部分，也能猜出来。
...
{doc}`../cnn-expedition/practice-peak/cnn-basics` 中我们学了 CNN 的编码过程：卷积+池化逐步**下采样**，空间尺寸从 28×28 缩小到 1×1。这是分类成功的关键——**丢掉位置信息，提炼语义信息**（{ref}`receptive-field` 告诉我们，深层神经元看的区域大，能理解整体）。
...
1. **编码器（Encoder）**：就是标准的 CNN，逐步下采样提取语义——跟你学的 LeNet-5（{doc}`../cnn-expedition/practice-peak/le-net`）前半部分一样
